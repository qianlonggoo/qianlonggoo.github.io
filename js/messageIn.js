...
function footerHide(iNub)
{
	...
	for(var i=0;i<aA.length;i++)
	{
		...
		aA[i].addEventListener("transitionend",function(ev){
			 ev.cancelBubble=true;
		},false);
	}
	oFooter.style.transition=".5s .5s";
	oFooter.style.WebkitTransition=".5s .5s";
	oFooter.style.MozTransition=".5s .5s";
	oFooter.style.MsTransition=".5s .5s";
	oFooter.style.WebkitTransition=".5s .5s";
	oFooter.addEventListener("webkitTransitionEnd",end,false);
	oFooter.addEventListener("transitionend",end,false);
...
function listHide(iNub)
{
	...
	for(var i=0;i<aLi.length;i++)//设置向下消失
	{
		...
		aLi[i].style.WebkitTransition=".5s "+(aLi.length-1)*100+"ms";
		aLi[i].style.MozTransition=".5s "+(aLi.length-1)*100+"ms";
		aLi[i].style.MsTransition=".5s "+(aLi.length-1)*100+"ms";
		aLi[i].style.OTransition=".5s "+(aLi.length-1)*100+"ms";
		aLi[i].style.opacity=0;
		aLi[i].style.marginTop="50px";
		aLi[i].addEventListener("webkitTransitionEnd",function(ev){
			 ev.cancelBubble=true;
		},false);
		aLi[i].addEventListener("transitionend",function(ev){
			 ev.cancelBubble=true;
		},false);
	}
	oList.style.transition="1s .5s";
	oList.style.WebkitTransition="1s .5s";
	oList.style.MozTransition="1s .5s";
	oList.style.MsTransition="1s .5s";
	oList.style.OTransition="1s .5s";
	oList.style.height="0px";
	oFooter.style.transition=".5s 1.5s";
	oFooter.style.WebkitTransition=".5s 1.5s";
	oFooter.style.MozTransition=".5s 1.5s";
	oFooter.style.MsTransition=".5s 1.5s";
	oFooter.style.OTransition=".5s 1.5s";
...
function showList()//让整个列表展开  但是li  还没有做特效
{
	...
	function end()
	{
		...
		oList.removeEventListener("transitionend",end,false);
		showLi();
...
function openLi(obj,iTime)
{
	...
	setTimeout(function(){
		...
		oBox.style.MozTransform="rotateY(0deg)";	
		oBox.style.MsTransform="rotateY(0deg)";	
		oBox.style.OTransform="rotateY(0deg)";	
	},iTime);
	function end()
	{
		this.removeEventListener("webkitTransitionEnd",end,false);
		...
		oReply.style.MozTransform="rotateX(0deg)";
		oReply.style.MsTransform="rotateX(0deg)";
		oReply.style.OTransform="rotateX(0deg)";
